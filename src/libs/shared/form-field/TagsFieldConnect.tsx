...
>({
  ...
  const handleAddTag = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (tags.length >= 10) return setError('tags', { message: '태그는 최대 10개까지 등록 가능합니다.' })
    if (e.currentTarget.value.length > 20) return setError('tags', { message: '태그는 20자 이내로 입력해 주세요.' })
    setValue('tags', [...tags, e.currentTarget.value], { shouldValidate: true })
