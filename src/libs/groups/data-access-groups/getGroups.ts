...
const getGroups = async ({ sortBy, keyword, isPublic, page = 1 }: GroupsSearchParams) => {
  const response = await getRequest<PaginationResponse<Group>>(`/groups?&keyword=${keyword}&sortBy=${sortBy}&isPublic=${isPublic}&page=${page}&pageSize=${isPublic ? PUBLIC_GROUPS_PAGE_SIZE : PRIVATE_GROUPS_PAGE_SIZE}`, { next: { tags: ['groups'] } })
  return response
}
