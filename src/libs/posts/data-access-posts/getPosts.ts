...
const getPosts = async (groupId: number, { sortBy, keyword, isPublic, page = 1 }: PostsSearchParams) => {
  // TODO-1: sortBy 백엔드 개발 완료 시 추가
  const response = await getRequest<PaginationResponse<Post>>(`/groups/${groupId}/posts?keyword=${keyword}&isPublic=${isPublic}&page=${page}&pageSize=${isPublic ? PUBLIC_POSTS_PAGE_SIZE : PRIVATE_POSTS_PAGE_SIZE}`, { next: { tags: ['posts'] } })
  return response
}
